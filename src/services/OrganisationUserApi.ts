...
export const adminRequests = async (token:string|undefined): Promise<AdminRequestsListTableResponseObject> => {
    let response = await axios.post('http://localhost:8080/admin/requestlist', { },{headers:{Authorization:`bearer ${token}`}})
    return response.data;
}

// Get Users List
export const getUsers = async (token:string|undefined): Promise<GetUsersResponse> => {
    let response = await axios.post('http://localhost:8080/admin/userslist', { },{headers:{Authorization:`bearer ${token}`}})
    return response.data;
}

// Get wfh
export const getUserWfh = async (token:string|undefined): Promise<GetUserWfhResponse> => {
    let response = await axios.post('http://localhost:8080/admin/userwfh', { },{headers:{Authorization:`bearer ${token}`}})
...
export const adminRejectRequest = async (organisationUserEmail: string | undefined, availedAt: Date | undefined, requestRejectionReason: string | undefined,token:string|undefined): Promise<AcceptRejectDeleteResponse> => {
    let response = await axios.post('http://localhost:8080/admin/rejectrequest', { organisationUserEmail,availedAt,requestRejectionReason},{headers:{Authorization:`bearer ${token}`}})
    return response.data;
}
...
export const adminAcceptRequest = async (organisationUserEmail: string | undefined, availedAt: Date | undefined,token:string|undefined): Promise<AcceptRejectDeleteResponse> => {
    let response = await axios.post('http://localhost:8080/admin/acceptrequest', { organisationUserEmail,availedAt },{headers:{Authorization:`bearer ${token}`}})
    return response.data;
}
...
export const requestWfh = async (availedAt: Date | undefined, requestSubmissionReason: string | undefined, token:string|undefined): Promise<AcceptRejectDeleteResponse> => {
    let response = await axios.post('http://localhost:8080/admin/requestwfh', {availedAt,requestSubmissionReason },{headers:{Authorization:`bearer ${token}`}})
    return response.data;
}

//Calender
export const requestCalenderData = async (token:string|undefined): Promise<CalendarResponse> => {
    let response = await axios.post('http://localhost:8080/admin/calender', { },{headers:{Authorization:`bearer ${token}`}})
    return response.data;
}